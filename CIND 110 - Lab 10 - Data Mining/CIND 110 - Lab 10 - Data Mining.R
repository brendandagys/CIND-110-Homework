...
#-------------------------------------------------------------------------------
#
# Title:      CIND110 Lab Exercise Module 10 Fall 2017
...
table(unlist(sapply (buglist, function (x) x$what), use.names = F))
table(unlist(sapply (buglist2, function (x) x$what)))
...
v <- unlist(sapply (buglist2, function (x) x$when[x$what == "NEW"]))
hist(as.POSIXct(v, origin = "1970-01-01"), breaks = "year")


# find the number of times tickets were re-opened
length(unlist(sapply (buglist, function (x) x$when[x$what == "REOPENED"])))
length(unlist(sapply (buglist2, function (x) x$when[x$what == "REOPENED"])))
...
v <- unlist(sapply (buglist2, function (x) if (length(which(x$what == "RESOLVED")) == 1) {x$when[x$what == "RESOLVED"] - x$when[x$what == "NEW"]}))
hist(v / (3600 * 24 * 7), main="Histogram of Weeks to Resolution", breaks = 800, xlim = c(0,20), xlab="Weeks to Resolution")
